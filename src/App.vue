...
<script lang="ts">
  ...
  import { createStory, nextInterestingCharacterFromStory } from './util/story'
  ...
  import { Tile as TileType } from '../data/types'

  // Step 1: Define an interface for your data structure
  interface AppData {
    gameOver: boolean
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      handleCollisions() {
        ...
        const confirmedCollisions: SpriteType[] = collideHeroAndSprites(this.hero.sprite, this.sprites, { debug: false })
      ...
      handleJumping() {
        const isDashJustStarted: boolean = jumpHeroAndDetectDash(this.hero, this.jumpButtonPressed)
      ...
      generateTiles() {
        ...
        const lastTile: TileType = this.tiles.slice(-1)[0] || { x: 0, width: 640 }
        if ( (lastTile.x - this.hero.sprite.x) < 640 ) {
          ...
          if ( isHeroAlive(this.hero) ) {
            ...
            const nextCharacter: string = nextInterestingCharacterFromStory(this.story)
            if ( nextCharacter ) {

              // Add the letter tile
              this.addTile(generateTileForCharacter(this.story.currentCharacter, lastTile.x + lastTile.width))

              // Translate to morse code, and add all those tiles now too
              const morseArray: string[] = textToMorseArray(nextCharacter)
              morseArray.forEach(morseCharacter => {
                const newLastTile: TileType = this.tiles.slice(-1)[0]
                this.addTile(generateTileForMorse(morseCharacter, newLastTile.x + newLastTile.width))
              })
            }
            else {
              // We reached the end, let's keep showing victory item until they touch it
              this.addTile(generateTileForMorse('🏆', lastTile.x + lastTile.width))
            }
          }
          else {
      ...
      addTile(tile: TileType) {
        ...
        this.sprites = [...this.sprites, ...tile.sprites]
      },
      pruneTiles() {
      ...
      generateTerrain() {
        ...
        const lastTerrain: SpriteType = this.terrain.slice(-1)[0] || { x: 0, width: 0, minDistance: 0 }
        if ((lastTerrain.x - this.hero.sprite.x) < 640 ) {
          const lastTerrain: SpriteType = this.terrain.slice(-1)[0]
          const terrainSprite: SpriteType = {...terrainLibrary.terrain, x: lastTerrain.x + lastTerrain.width}
          this.terrain = [...this.terrain, terrainSprite]
        }
      },
      doCommand(e) {
        let cmd: string = String.fromCharCode(e.keyCode).toLowerCase()
        if ( cmd === ' ') {
          this.onJumpPress()
        }
      },
      ...
      onGameOver() {
        ...
        // This helps us get to show the grave faster
